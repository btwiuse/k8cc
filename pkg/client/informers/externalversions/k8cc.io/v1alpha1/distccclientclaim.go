...
func NewDistccClientClaimInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredDistccClientClaimInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredDistccClientClaimInformer constructs a new informer for DistccClientClaim type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredDistccClientClaimInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		...
		&k8cciov1alpha1.DistccClientClaim{},
		resyncPeriod,
		indexers,
	)
}

func (f *distccClientClaimInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredDistccClientClaimInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *distccClientClaimInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&k8cciov1alpha1.DistccClientClaim{}, f.defaultInformer)
}
